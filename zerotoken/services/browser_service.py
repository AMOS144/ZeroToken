...
class BrowserService:
    ...
    async def open(self, url: str, wait_until: str = "networkidle", **kw: Any) -> OperationRecord:
        ...
        return await pipeline.execute(
            ...
            action_fn=open_action,
            needs_selector=False,
            take_screenshot=kw.get("take_screenshot", True),
        )

    async def wait_for(
        self, condition: str, value: str | None = None, **kw: Any
    ) -> OperationRecord:
        pipeline = self._ensure_pipeline()
        params = {
            "condition": condition,
    ...
    async def scroll(
        self, direction: str = "down", amount: int = 300, selector: str | None = None, **kw: Any
    ) -> OperationRecord:
        pipeline = self._ensure_pipeline()
    ...
    async def upload(self, selector: str, path: str | list[str], **kw: Any) -> OperationRecord:
        pipeline = self._ensure_pipeline()
        return await pipeline.execute(
    ...
    async def download(
        self, selector: str, save_dir: str | None = None, **kw: Any
    ) -> OperationRecord:
        pipeline = self._ensure_pipeline()
        return await pipeline.execute(
            ActionType.FILE_DOWNLOAD,
            {"selector": selector, "save_dir": save_dir},
            action_fn=lambda _f, _e, p: download_action(self._context.active_page.page, _e, p),
            auto_save=kw.get("auto_save", False),
            adaptive=kw.get("adaptive", False),
            identifier=kw.get("identifier"),
            take_screenshot=kw.get("take_screenshot", True),
