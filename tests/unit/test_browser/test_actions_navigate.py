...
async def test_open_action():
    ...
    result = await open_action(
        mock_frame, None, {"url": "https://example.com", "wait_until": "networkidle"}
    )
    mock_frame.goto.assert_called_once_with(
...
async def test_wait_for_action_selector():
    ...
    mock_frame = AsyncMock()
    result = await wait_for_action(
        mock_frame, None, {"condition": "selector", "value": "#btn", "timeout": 5000}
    )
